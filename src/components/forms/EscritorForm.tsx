...
import { Sparkles, PenTool, Layers, Loader2 } from 'lucide-react';
import { cn } from '@/src/lib/utils';
import { ShiningText } from '@/src/components/ui/shining-text';

interface EscritorFormProps {
  initialTopic?: string;
  onSubmit: (data: { topic: string; chapters: number; language: string }) => void;
  loading: boolean;
}

const CHAPTERS_OPTIONS = [4, 8, 12, 16];
const LANGUAGES = ['Português (Brasil)', 'Espanhol', 'Inglês'];

export function EscritorForm({ initialTopic = '', onSubmit, loading }: EscritorFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSubmit({ topic, chapters, language });
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="space-y-4">
        <label className="text-[10px] font-bold tracking-widest text-[var(--text-secondary)] uppercase">
          Título, Tema ou Referência do Livro
        </label>
        <div className="relative group">
          <input
            ...
            placeholder="Ex: 'O Poder da Oração' ou 'Estudo em Romanos'"
          ...
          />
          <PenTool className="absolute left-4 top-1/2 -translate-y-1/2 text-[var(--text-secondary)] group-focus-within:text-yellow-500 transition-colors" size={20} />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            >
              {CHAPTERS_OPTIONS.map(opt => (
                <option key={opt} value={opt} className="bg-[var(--bg-card)]">{opt} Capítulos</option>
              ))}
            </select>
